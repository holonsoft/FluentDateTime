...
using holonsoft.FluentDateTime.DateTimeOffset;
using System;
...
namespace Tests
{
	public class TestDateTimeOffset
	{
		...
		public void Ago_FromFixedDateTime_Tests(int agoValue)
		{
			...
			Assert.Equal(agoValue.Weeks().Before(originalPointInTime), originalPointInTime.AddDays(-agoValue*_daysPerWeek));
		...
		public void From_FromFixedDateTime_Tests(int value)
		{
			...
			Assert.Equal(value.Weeks().From(originalPointInTime), originalPointInTime.AddDays(value*_daysPerWeek));
		...
		public void IsBefore_ReturnsTrueForGivenDateThatIsInTheFuture()
		{
			...
			// assert
			Assert.True((bool) DateTimeOffset.UtcNow.IsBefore(toCompareWith));
		}
		...
		public void IsAfter_ReturnsTrueForGivenDateThatIsInThePast()
		{
			...
			// assert
			Assert.True((bool) DateTimeOffset.UtcNow.IsAfter(toCompareWith));
		}
		...
		[Fact]
		public void TestUtcOffsetInMinutes()
		{
			var expected = 60;
			...
			Assert.Equal(expected, DateTime.Now.UtcOffsetInMinutes());
		}
