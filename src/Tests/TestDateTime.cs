...
namespace Tests
{
	public class TestDateTime
	{
		...
		public void Ago_FromFixedDateTime_Tests(int agoValue)
		{
			...
			DateAssert.Equal(agoValue.Weeks().Before(originalPointInTime), originalPointInTime.AddDays(-agoValue * _daysPerWeek));
		...
		public void From_FromFixedDateTime_Tests(int value)
		{
			...
			DateAssert.Equal(value.Weeks().From(originalPointInTime), originalPointInTime.AddDays(value * _daysPerWeek));
		...
		public void IsBefore_ReturnsTrueForGivenDateThatIsInTheFuture()
		{
			...
			// assert
			Assert.True((bool) DateTime.Today.IsBefore(toCompareWith));
		}
		...
		public void IsBefore_ReturnsFalseForGivenDateThatIsSame()
		{
			...
			// assert
			Assert.False((bool) toCompareWith.IsBefore(toCompareWith));
		}
		...
		public void IsAfter_ReturnsTrueForGivenDateThatIsInThePast()
		{
			...
			// assert
			Assert.True((bool) DateTime.Today.IsAfter(toCompareWith));
		}
		...
		public void IsAfter_ReturnsFalseForGivenDateThatIsSame()
		{
			...
			// assert
			Assert.False((bool) toCompareWith.IsAfter(toCompareWith));
		}
		...
		{
			var now = DateTime.Now;
			Assert.False((bool) now.Subtract((TimeSpan) 2.Seconds()).IsInFuture());
			Assert.False((bool) now.IsInFuture());
			Assert.True((bool) now.Add(2.Seconds()).IsInFuture());
		}
		...
		{
			var now = DateTime.Now;
			Assert.True((bool) now.Subtract((TimeSpan) 2.Seconds()).IsInPast());
			Assert.False((bool) now.Add(2.Seconds()).IsInPast());
		}
		...
		public void SameYear_Y(string dateValue)
		{
			...
			Assert.True((bool) date.SameYear(other));
		}
		...
		public void SameYear_N(string dateValue)
		{
			...
			Assert.False((bool) date.SameYear(other));
		}
		...
		public void SameMonth_Y(string dateValue)
		{
			...
			Assert.True((bool) date.SameMonth(other));
		}
		...
		public void SameMonth_N(string dateValue)
		{
			...
			Assert.False((bool) date.SameMonth(other));
		}
		...
		public void SameDay_Y(string dateValue)
		{
			...
			Assert.True((bool) date.SameDay(other));
		}
		...
		public void SameDay_N(string dateValue)
		{
			...
			Assert.False((bool) date.SameDay(other));
		}
		...
		public void TestWeekToDate()
		{
			...
			var first = date.FirstDayOfCalenderWeekISO8601(2009, date.GetGermanCalendarWeek().Week);
			var last = date.LastDayOfCalenderWeekISO8601(2009, date.GetGermanCalendarWeek().Week);

			Assert.Equal(new DateTime(2009, 12, 28), first);
			Assert.Equal(new DateTime(2010, 1, 3), last);
		}
		...
		public void TestRandomDates()
		{
			...
			var dateDict = new List<DateTime>();

			var r1 = DateTime.Now.RandomDateInFuture(now.AddDays(100));
			Assert.True(r1 >= now);
			var r2 = DateTime.Now.RandomDateInFuture(now.AddDays(100));
			Assert.True(r2 >= now);
			var r3 = DateTime.Now.RandomDateInFuture(now.AddDays(100));
			Assert.True(r3 >= now);
			var r4 = DateTime.Now.RandomDateInFuture(now.AddDays(100));
			Assert.True(r4 >= now);


			var r10 = DateTime.Now.RandomDateInPast(now.AddDays(-100));
			Assert.True(r10 <= now);
			var r11 = DateTime.Now.RandomDateInPast(now.AddDays(-100));
			Assert.True(r11 <= now);
			var r12 = DateTime.Now.RandomDateInPast(now.AddDays(-100));
			Assert.True(r12 <= now);


			dateDict.AddRange(new DateTime[] { r1, r2, r3, r4, r10, r11, r12 });

			Assert.True(dateDict.Distinct().ToList().Count > 1);
		}
		...
		{
			var date = new DateTime(2021, 01, 18, 13, 14, 15, DateTimeKind.Utc);

			var unixDateTimeLong = date.ToUnixTimeSeconds();
			var restoredDate = unixDateTimeLong.FromUnixTimeSecondsToDateTime();
			Assert.Equal(date, restoredDate);


			var unixDateTimeULong = (ulong) date.ToUnixTimeSeconds();
			restoredDate = unixDateTimeULong.FromUnixTimeSecondsToDateTime();
			Assert.Equal(date, restoredDate);
